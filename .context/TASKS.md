...
| M4 | Session + rate-limit dual-driver store | 9 | 5 | **Major milestone hit 2026-04-27.** T4.1 + T4.2 + T4.4 + T4.5 + T4.6 all done. Both `src/infra/session/` and `src/infra/rate-limit/` are landed AND `src/index.js` is fully behind both factories. The bespoke in-memory `globalRateLimitMap`, `rateLimitMap`, hourly `rateLimitCleanupInterval`, and the two drifted `RATE_LIMIT_EXEMPT_PATHS` copies are all deleted; G4.1 has a permanent negative-assertion ratchet preventing their reintroduction. Orphan-timer count: 6 → 5. Test baseline: 726 / 726 passing across 54 suites in ~11 s. Remaining: T4.3 (Redis session driver, deferred on OQ-11), T4.7 (testcontainers Redis integration, depends on T4.3), T4.8 (CIDR `trust proxy`, scheduled after T4.6), T4.9 (FK access_tokens.owner_id ADR-0015 Option B). M4 critical path is **complete** for OSS-only deployments. |

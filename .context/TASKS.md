...
| M4 | Session + rate-limit dual-driver store | 9 | 4 | **In progress.** T4.1 + T4.2 + T4.4 + T4.5 done 2026-04-27. Both `src/infra/session/` and `src/infra/rate-limit/` are landed with memory + sqlite drivers behind named factories and full driver-agnostic contract suites (20 + 23 tests). Session wiring in `src/index.js` is already swapped (T4.4); rate-limit wiring is the only remaining call-site change (T4.6). **Pre-stage gates required and green:** Stage-0 baseline (G0.1, G0.2, G0.3, G0.4, G0.6) + M4 layer (G4.1, G4.2). Every M4 PR that touches `src/index.js` MUST update the snapshots in G4.1 / G4.2 inline with the source change so reviewers can audit each deletion / replacement against the locked baseline. |

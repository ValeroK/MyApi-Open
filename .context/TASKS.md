...
| M4-pre | M4 pre-stage gates (G4.1–G4.3) | 3 | 3 | ✅ **Complete 2026-04-27** — see ADR-0019 §"Per-milestone follow-ups". G4.1 pins rate-limit implementation source + 429 envelope; G4.2 pins session config + cookie attributes + persistence/regenerate/destroy invariants; G4.3 pins `app.set('trust proxy', 1)` source-line region + spoofability behavior probe + negative ratchets on `TRUSTED_PROXIES` / `parseTrustedProxies` / `src/lib/trust-proxy.js` (all flip post-T4.8). Surfaced finding: two drifted exempt-path lists in `src/index.js` — M4 consolidated in T4.6. Test baseline jumped 668 → 732 across the M4 work / 22 skip / 55 suites, exit 0. |
| M4 | Session + rate-limit dual-driver store | 9 | 6 | **Major milestone hit 2026-04-27.** T4.1 + T4.2 + T4.4 + T4.5 + T4.6 + **T4.8** all done. Both `src/infra/session/` and `src/infra/rate-limit/` are landed AND `src/index.js` is fully behind both factories. The bespoke in-memory `globalRateLimitMap`, `rateLimitMap`, hourly `rateLimitCleanupInterval`, and the two drifted `RATE_LIMIT_EXEMPT_PATHS` copies are all deleted; G4.1 has a permanent negative-assertion ratchet preventing their reintroduction. Orphan-timer count: 6 → 5. **T4.8 closed H5**: `app.set('trust proxy', 1)` → `app.set('trust proxy', parseTrustedProxies(process.env.TRUSTED_PROXIES))` with secure-by-default `['loopback']`; `req.ip` no longer spoofable from arbitrary public clients. Test baseline: 767 / 789 passing across 55 suites in ~11 s. Remaining: T4.3 (Redis session driver, deferred on OQ-11), T4.7 (testcontainers Redis integration, depends on T4.3), T4.9 (FK access_tokens.owner_id ADR-0015 Option B — different layer entirely). M4 OSS-critical-path is **complete** including the H5 hardening. |
...
| F6 | Agent capability verification (L1 supertest + L2 live-smoke + L3 runbook + connector feasibility + gap ledger) | 8 | 8 | ✅ **Complete 2026-04-28** — F6.0 (ledger bootstrap) + F6.1 (8 L1 supertest suites, +81 tests, GAP-002 P0 closed in same change, 4 new gaps documented) + F6.2 (L3 runbook + `scripts/agent-walkthrough.mjs`) + F6.3 (Google L2 live-smoke) + F6.4 (GitHub L2 live-smoke) + F6.5 (skills + handshake L2 live-smoke) + F6.6 (connector spike + JSON schema validator + handler wiring per ADR-0020) + F6.7 (gap-ledger triage; F8 / F9 / F10 follow-up briefs filed). Test baseline: 55 / 56 / 767 → **64 / 69 / 855** (+9 suites, +88 tests, 5 L2 suites correctly skip when env unset, 28 / 28 snapshots stable). |
| F11 | 2FA reset mechanism — master-only admin endpoint + dashboard surface + setup-guard against silent secret rotation. Filed 2026-04-29 from a real lockout incident (`mailer.kv@gmail.com`); recovery today is hand-edit SQLite, which is unsupportable for non-operator-owned deployments. Effort: S (half a day to one day). | 1 | 0 | Not started — see `.context/tasks/backlog/F11-2fa-reset-mechanism.md`. |
| F12 | Rolling upstream-backport batch (open). Mini-plan-gated cherry-picks from `omribenami/MyApi-Open` since fork-point `725060bb`. Each candidate has a `.context/imports/<hash>-<slug>.md` mini-plan with explicit "do we want it / why / risks" before any code lands. Direct-to-main commits, gated on the v0.6.0 test baseline (71/77/898/exit-0). See `.context/tasks/backlog/F12-upstream-import-2026-05.md` and ADR-0024. | 22 | 0 | In progress — bootstrap landed 2026-05-04 (ADR-0024 + audit script + 18 mini-plans). Triage pending per mini-plan. |
| M-import | Rolling upstream backports milestone — tracks merged candidates from F12 only. Sub-rows added incrementally as F12 mini-plans flip to `accepted`. Targets v0.7.0 release per ADR-0023. | 0 | 0 | In progress (rows added on accept). |
| **Total** |  | **159** | **37** |  |
...
| # | Task | Effort | Depends on | Notes |
|---|------|--------|------------|-------|
| T4.1 | `[x]` Define `SessionStore` interface + tests in `src/infra/session/SessionStore.js` (presence-only shape check; arity dropped because both `MemoryStore` and `better-sqlite3-session-store` use default-param signatures) | S | T0.7 | Done 2026-04-27. New 20-test contract suite at `src/tests/cleanup-m4-session-store-contract.test.js` runs every test against memory + sqlite drivers. |
| T4.2 | `[x]` Implement `SqliteSessionStore` (wraps `better-sqlite3-session-store`) + memory driver + `createSessionStore({ env, db })` factory under `src/infra/session/` | M | T4.1 | Done 2026-04-27. Wrapper neutralizes upstream timer leak (intercepts `setInterval` during construction → captures handle → `.unref()` + exposes `.close()`). Factory selection rule preserves today's inline behavior in `src/index.js` so T4.3 is a pure refactor. |
| T4.3 | `[ ]` Implement `RedisSessionStore` using `connect-redis` + `ioredis` (add deps); TLS required if URL is `rediss://` | M | T4.1, OQ-11 | **Deferred** until OQ-11 (Redis TLS policy) is resolved. Factory throws a clear "T4.6 not yet implemented" error if `REDIS_URL` is set today, so misconfiguration is loud not silent. |
| T4.4 | `[x]` Replace inline `sessionStore` wiring in `src/index.js` with `createSessionStore({ env, db })` from `src/infra/session/` | S | T4.2 | Done 2026-04-27. Net +6 lines in `src/index.js` (24 ins / 18 del). `setAuthHardeningSessionStore(...)` now receives the store only when `driver === 'sqlite'` to preserve today's "no Store handle in test mode" behavior bit-for-bit. Snapshot updates: 1 in G4.2 (intentional — `app.use(session({...}))` block lost its conditional spread now that `sessionStore` is always defined), 5 in G4.1 + G0.3 (line-number drift only — bodies byte-identical). |
| T4.5 | `[x]` Define `RateLimitStore` interface (`consume / reset / close`) + Memory + Sqlite drivers under `src/infra/rate-limit/` + 23-test driver-agnostic contract suite | M | T4.1 | Done 2026-04-27. **Semantic switch: sliding-log → fixed-window.** Sliding-log allocates per request and is O(req) on SQLite; fixed-window is O(1). G4.1 pins the 429 envelope shape, not burst-precision, so this trade-off is in-contract. SqliteRateLimitStore creates its own `rate_limit_counters` table on construction (same pattern as `better-sqlite3-session-store`) — no migration file needed for T4.5. Both drivers expose `.unref()`'d GC timers + idempotent `.close()`. Redis driver throws "not yet implemented" loudly when `REDIS_URL` is set. |
| T4.6 | `[x]` Replace bespoke rate-limit maps + `rateLimitCleanupInterval` + drifted `RATE_LIMIT_EXEMPT_PATHS` lists with store-backed wiring; rewrite `function rateLimit(...)` and the inline global middleware over `rateLimitStore.consume(...)` | M | T4.5 | Done 2026-04-27. Net **−4 lines** in `src/index.js` (92 ins / 96 del) but the diff is structurally larger — the 28-line `rateLimitCleanupInterval` setInterval is **gone** (orphan-timer count 6 → 5; G0.3 ratchet acknowledged), `globalRateLimitMap` and `rateLimitMap` are **gone** (G4.1 negative-assertion test ratchets that they cannot return), and the two drifted exempt-path lists collapse to a single `RATE_LIMIT_EXEMPT_PATHS` + `isRateLimitExempt(req)` helper at the top of the file. HTTP envelopes preserved bit-for-bit per call site (3 distinct ones: bearer, IP-global, per-namespace). G4.1 test surgery: deleted 2 dead snapshots (`globalRateLimitMap`, `rateLimitCleanupInterval`), added 2 new ones (`store-backed global limiter source` snapshot + `M4-T4.6 deletions: legacy in-memory rate-limit symbols are gone` negative-assertion ratchet using `stripComments`). |
...
| T4.8 | `[x]` Tighten `app.set('trust proxy', ...)` to a CIDR list via env (`TRUSTED_PROXIES`); default to loopback only | S | T4.6 | Done 2026-04-27. New `src/lib/trust-proxy.js` with `parseTrustedProxies(envValue)` (regex-based, 0-dep, fails loud on bad input). G4.3 expanded 6 → 41 tests covering 28 input shapes, 9 fail-loud cases, error message echoes verbatim, axiom tests preserved (trust=1 honors XFF, trust=false ignores XFF), 6 integration probes against Express's compiled `trust proxy fn` (`127.0.0.1` ✓, `203.0.113.42` ✗, `uniquelocal` does NOT auto-include loopback). G0.2 snapshot pins `'trust proxy': ['loopback']`. `.env.smoke.example` gained a TRUSTED_PROXIES doc block. **Closes H5** from plan.md §6.3. |
...
## F6 — Agent capability verification 🟡 In progress (2026-04-28)

**Goal.** Map every agent-facing capability and verify it end-to-end in
three test layers (L1 in-process supertest, L2 gated live-smoke against
`docker:smoke`, L3 manual agent-as-user runbook), record every gap surfaced
along the way in `.context/capability-gaps.md`, and produce a feasibility
ADR for "agent creates a new connector at runtime". Closes the
"is MyApi ready for OpenClaude / Hermes / etc.?" question by giving every
agent-facing capability a green test and naming every known limitation.

**Exit criteria.**
- `.context/capability-gaps.md` exists with the schema + pre-seeded
  findings + at least all `open` rows triaged into M-numbered milestones
  or follow-up briefs.
- 8 new L1 supertest suites under `src/tests/` cover the 8 agent-facing
  surfaces enumerated in
  `.cursor/plans/capability_test_plan_b4523725.plan.md` §3.1.
- 5 new L2 live-smoke suites (`*-live-smoke.test.js`) gated on
  `SMOKE_URL` + per-suite env vars, runnable via
  `npm run smoke:agent` / `smoke:google` / `smoke:github`.
- `.context/runbooks/agent-real-life.md` + `scripts/agent-walkthrough.mjs`
  exist and have been walked end-to-end at least once with sign-off
  recorded in `.context/sessions/`.
- `ADR-0020-agent-defined-connectors-feasibility.md` accepted; the
  master-only connector storage round-trip is locked behind
  `src/tests/connectors-spike.test.js`.
- `npm test` is green at ≥ 55 suites with ≥ 767 + (F6 additions) passing.

**Depends on.** M3 (closed; provides OAuth groove + `docker:smoke`),
M4 OSS critical path (closed; provides session + rate-limit contract
gates).
**Blocks.** Nothing on the existing roadmap — F6 is **insurance** before
external agent integration.
**Related.** `M5` (SSRF unification — F6.6's connector spike intentionally
stops short of runtime adapter loading because that is M5's domain),
`M6` (monolith extraction — F6 tests will re-anchor on new module paths
when M6 lands), `M9` / `M14` (the gap ledger feeds both).

| # | Task | Effort | Depends on | Notes |
|---|------|--------|------------|-------|
| F6.0 | `[x]` Bootstrap `.context/capability-gaps.md` (schema + severity guide + pre-seeded `GAP-001..GAP-007` from the exploration pass) | XS | — | Done 2026-04-28 (markdown-only; landed in plan mode). |
| F6.1 | `[x]` 8 L1 supertest behavioral suites under `src/tests/` (`google-mount-auth-posture` — closes GAP-002 P0 + regenerates G0.1/G0.2/G0.3 snapshots; `agent-discovery-contract`; `services-proxy-behavioral` — surfaces GAP-008 + GAP-009; `services-execute-behavioral` — surfaces GAP-010; `ask-endpoint-behavioral`; `handshake-flow-behavioral` — surfaces GAP-011 + pins GAP-005; `connectors-master-only`; `agent-capabilities-end-to-end`) | L | F6.0, M4 | **Done 2026-04-28.** All 8 suites green; **+81 passing tests** (5+12+12+9+8+16+8+11). Test baseline: 55/56 suites + 767 passing → **63/64 suites + 848 passing**, 28/28 snapshots stable, exit 0, full-sweep runtime ~64s on Windows. No `nock` dep introduced — auth/scope/validation/connection gates can be exercised without upstream mocks; happy-path with real upstream is owned by L2 (F6.3+). Static gates added: google-mount source-line ratchet, services-proxy req.body destructure ratchet (SSRF resilience by construction), handshake duplicate-status-handler count ratchet. |
| F6.2 | `[x]` `.context/runbooks/agent-real-life.md` runbook (filed in plan mode) + `scripts/agent-walkthrough.mjs` (uses only a bearer token; runs the runbook's curls programmatically) | M | F6.0 | **Done 2026-04-28.** `scripts/agent-walkthrough.mjs` walks Phase 0 (sanity + scope-introspection footgun warning for GAP-008), Phase 3 (discovery), Phase 4 (Google + GitHub credential-less call with credential-leak check, gated on `SMOKE_GOOGLE=1` / `SMOKE_GITHUB=1`), Phase 5 (master-only boundary), Phase 6 (5 SSRF probes), Phase 7 (full handshake e2e if `SMOKE_MASTER` is set), Phase 8 (skills metadata). Each step prints `OK §N.x` / `FAIL §N.x` with status + latency; FAIL lines are actionable templates for `capability-gaps.md`. New `npm run smoke:walkthrough` invokes it. |
| F6.3 | `[x]` L2 Google live-smoke: `agent-discovery-live-smoke.test.js` + `services-proxy-google-live-smoke.test.js`; new `npm run smoke:agent` + `smoke:google` scripts | M | F6.1, M3 (closed) | **Done 2026-04-28.** Suite skipped silently in `npm test` (gated on `SMOKE_URL` + `SMOKE_BEARER` + `SMOKE_GOOGLE=1`); shows up as 1 of 5 skipped suites. Asserts `/openapi.json`, `/llms.txt`, `/api/v1/capabilities` (with bearer), `/api/v1/tokens/me/capabilities` (no raw bearer in body), `/api/v1/gateway/context` → 403 (GAP-003 hold), and a Gmail profile read with credential-leak markers. |
| F6.4 | `[x]` L2 GitHub live-smoke: `services-proxy-github-live-smoke.test.js` + `npm run smoke:github` | M | F6.3 | **Done 2026-04-28.** Verifies F4 identity-vs-service scope split (ADR-0018) against a real GitHub app — `/user` works on identity scope; `/user/emails` accepts `200`/`403`/`404` since identity-only grants lack `user:email`. SSRF probe + credential-leak markers checked too. |
| F6.5 | `[x]` L2 handshake + skills live-smoke: `skills-and-handshakes-live-smoke.test.js` | M | F6.3 | **Done 2026-04-28.** Walks the full public-POST → public-poll(pending) → master-approve → public-poll(approved) flow over HTTP and asserts the issued token never appears in the public poll body. Skills metadata read accepts 403 (no `skills:read` scope on the smoke bearer) as a graceful skip. _Note: `oauth-connect-flow-live-smoke.test.js` (gated on `SMOKE_INTERACTIVE=1`) deferred — the M3 wrap-up's live Google smoke (`sessions/2026-04-24-m3-smoke.md`) already covered the connect flow end-to-end against the shipped binary._ |
| F6.6 | `[x]` Connector feasibility: `ADR-0020-agent-defined-connectors-feasibility.md` (filed in plan mode 2026-04-28) + `src/tests/connectors-spike.test.js` (master vs scoped 403, round-trip, malformed 400) + `src/lib/schemas/connector-spec.js` (`GenericOAuthAdapter` validator, 0-dep, F4 identity-vs-service split enforced) + handler wiring at `src/index.js:6389` + `F7` task brief filed for runtime loader + workflow + marketplace `connector` type (gated on M5) | S | F6.0 | **Done 2026-04-28.** ADR ratifies "Option C — read-only spike now, runtime work blocked on M5". 7 spike tests green: master round-trip; missing `tokenUrl` → 400 INVALID_CONNECTOR_SPEC; bogus URL → 400; identity ∩ service overlap → 400; both-scope-bags-missing → 400; scoped → 403 BEFORE schema gate; non-`generic_oauth` types bypass schema (today's behavior, pinned). Snapshot drift on G0.3 (timer line numbers +19) regenerated in same change per ADR-0019. |
| F6.7 | `[x]` Triage pass on `.context/capability-gaps.md` — walk every `open` row, classify into M-numbered milestones, file follow-up briefs, bump global progress in this file, update `.context/current_state.md` §5 + §6 | S | F6.1–F6.6 | **Done 2026-04-28.** Triage summary table at top of `capability-gaps.md`. New briefs filed: `F8-gateway-context-vs-llms-txt-mismatch.md` (GAP-003), `F9-scope-hierarchy-engine.md` (GAP-008+009), `F10-service-catalog-seed.md` (GAP-010). Existing-milestone bundles: GAP-001 / 006 → M14; GAP-005 / 011 → M6; GAP-007 → accept-risk (already covered by ADR-0016 follow-up); GAP-004 → F7 (deferred per ADR-0020). GAP-002 stays `resolved`. Net: zero `triage` rows remain. |

---

## M-import — Rolling upstream backports 🟡 In progress (open)

**Goal.** Selectively cherry-pick changes from `upstream/omribenami/MyApi-Open`
into the fork using the per-import workflow + mini-plan gate ratified by
ADR-0024. Targets v0.7.0 release once the first batch of accepted imports
lands (cadence per ADR-0023).

**Exit criteria.** Defined per accepted candidate in
`.context/tasks/backlog/F12-upstream-import-2026-05.md`. Milestone closes
when every mini-plan in `.context/imports/` is at status
`merged | deferred | rejected` AND the v0.7.0 tag has been cut.

**Depends on.** ADR-0024 (workflow), ADR-0023 (release pipeline + v0.6.0
rollback target), the v0.6.0 test baseline (71/77 suites, 898 tests,
28/28 snapshots, exit 0).
**Blocks.** Nothing on the existing roadmap — F12 is selective uplift,
not a critical path.
**Related.** F12 parent task (`.context/tasks/backlog/F12-...`); per-candidate
mini-plans under `.context/imports/`.

**Sub-row policy.** Empty until first mini-plan accepts. Each accepted
mini-plan adds one row here AND one row in F12 with matching status.
Architectural imports (52dd343, e0707c5, 6ab11cb, a6da498, f5297d5)
require their respective ADR (0025-0029) to land in the same commit
as the import.

| # | Task | Effort | Depends on | Notes |
|---|------|--------|------------|-------|
| _(empty until first accept)_ | | | | |
...
| 2026-04-30 | implementation | **TOTP verify window widened from `window: 2` (±60 s) → `window: 4` (±120 s) at every verify call site; `TOTP_CODE_TTL_MS` raised 90 s → 270 s.** Direct follow-up to the 2026-04-29 `mailer.kv@gmail.com` lockout: even after the operator wiped `data/myapi.db` clean (5 files), restarted the container, re-signed up via Google, and scanned a fresh QR, every verify attempt still returned `Invalid 2FA code`. Live `verifyDelta` against the running container (read in-process via a second `better-sqlite3` connection inside the container so the live app's WAL writes are visible — host-side reads get masked by the container's open WAL on Windows Docker bind mounts) proved the user's phone was generating codes for `delta = +4 steps = +120 s` ahead of server. The previous `window: 2` only accepts ±60 s, so every honest code was rejected. QR generation, secret storage, container clock, session auth, account routing, and replay-tracker logic were all verified correct under direct probe — this was a clock-tolerance problem, not a code bug. **Code changes:** 4 sites in `src/index.js` (`/auth/2fa/verify`, `/auth/2fa/disable`, `/auth/2fa/challenge`, `/admin/security/rotate-key`) + 1 site in `src/routes/auth.js` (`POST /auth/login` 2FA gate) flipped `window: 2` → `window: 4`; `TOTP_CODE_TTL_MS` in `src/lib/authHardening.js` raised `90_000` → `270_000` (240 s validity span at `window: 4` + 30 s buffer for clock skew between the verify call and the replay-mark write); inline comment block updated to point at the new source-pin test. **Decision rationale + brute-force math + revisit trigger** are recorded in `ADR-0022-totp-window-tolerance.md` (Option B chosen over an env-var knob; revisit when M11+ multi-tenant SaaS shape forces per-tenant clock-drift policy). **Test-first discipline:** new `src/tests/totp-window-tolerance.test.js` (6 tests, written red-first against the pre-fix tree — 5 fail / 1 pass — then flipped green by the bumps) source-pins the production constants on every verify block and the replay TTL constant. Coverage: behavioural at `±90 s` (accepted) + `+150 s` (rejected) via supertest against `POST /api/v1/auth/login` with `totpCode`; static gate that fails if any `speakeasy.totp.verify({...})` block in either `src/index.js` or `src/routes/auth.js` regresses to `window: 2` or below; static gate that fails if `TOTP_CODE_TTL_MS` drops below `240_000`. Test baseline: 66 / 71 / 870 → **67 / 72 / 876** (+1 suite, +6 tests, no regressions, 28 / 28 snapshots stable, exit 0, 61 s on Windows with `--forceExit`). Does NOT close F11 — that brief still tracks the master-only `POST /api/v1/admin/users/:id/2fa/reset` endpoint + the defensive `409 ALREADY_ENROLLED` guard on `/auth/2fa/setup`, both of which remain unaddressed. Lint + typecheck: zero new findings on the four files touched; pre-existing `tsc` errors all on lines outside the changes. |
| 2026-04-29 | ops | **2FA reset for `mailer.kv@gmail.com` + F11 brief filed.** User reported 2FA codes rejected at `/auth/2fa/challenge` (six `2fa_failed_attempt` rows on 2026-04-29; last successful `2fa_challenge_passed` was 2026-04-28). Investigation confirmed no recent code regression — F5.1 (the only commit that reworked 2FA, on 2026-04-25) only touched the password-login path, but the user is on Google OAuth (`/auth/2fa/challenge`), and the OAuth-side TOTP verification has been byte-stable. Most likely cause: phone-side authenticator entry drift (their audit showed two `2fa_setup_started` rows ~45 s apart on 2026-04-25 — `setUserTotpSecret` silently overwrites on each setup, so scanning the FIRST QR leaves the DB holding the SECOND secret). Operator-side fix applied via direct SQL: `UPDATE users SET two_factor_enabled = 0, totp_secret = NULL WHERE id = 'usr_b39141282ade254314940d2965a8ab8a'`; verified `before / after` rows + `1 row updated`. **Second-step finding:** the user reported the prompt was still showing after the DB reset. Inspection of `data/sessions.sqlite` revealed stale rows carrying a frozen `pending_2fa_user.twoFactorEnabled: true` snapshot from the OAuth-callback stash at `src/index.js:8708`. Deleted those rows. **Third-step finding (the actual root cause):** even after the host DB write AND the session-clear, *fresh* OAuth callbacks at 15:46 and 15:48 kept writing new `pending_2fa_user` rows. Diagnosis via `/proc/95/fd/` inside the running `myapi-smoke` container revealed `myapi.db-wal` and `myapi.db-shm` held open inside the container but **invisible to the host's `ls`** (Windows Docker bind-mount semantics). The host-side `UPDATE` went into the base file `myapi.db`, but the running container's `getUsers()` was reading from its own WAL-shadowed snapshot view that masked the host write — so the container still saw `twoFactorEnabled: true` and routed every callback through `pending_2fa`. Resolution: `docker restart myapi-smoke` (clean WAL checkpoint + reopen of the bind-mounted base file). Verified post-restart by tailing the container log: `[2026-04-29T16:11:42.074Z] INFO: [OAuth Callback] routing=fast_path_returning` (was `routing=pending_2fa` for every prior attempt). Cleared one residual session post-restart for completeness. **F11 brief updated** with two non-negotiable requirements discovered in this incident: (a) the admin endpoint MUST scrub active sessions in the same call (the DB write alone leaves stale `pending_2fa_user` rows in `sessions.sqlite`), and (b) the admin endpoint MUST execute through the app's own running DB connection — a host-side or sidecar `UPDATE users` is silently masked by the container's open WAL on Windows Docker bind mounts (`myapi.db-wal` + `myapi.db-shm` are held by the running node process but invisible to `ls` on the host). A supertest-driven L1 must explicitly cover the "host-edit-while-app-running" anti-pattern. **No code change.** Global progress total bumped 136 → 137; F11 row added at 0/1. |
| 2026-04-28 | implementation | **F6 — Agent capability verification ✅ Complete.** All 8 tasks (F6.0–F6.7) landed in a single working session. **F6.0** ledger bootstrap (markdown only, plan mode). **F6.1** 8 L1 supertest behavioral suites: `google-mount-auth-posture` (closes GAP-002 P0 — `/api/v1/google/*` mount was naked; `resolveUserId` falls back to literal `'owner'`; one-line `authenticate` middleware fix + 5-test ratchet + G0.1 / G0.2 / G0.3 snapshots regenerated per ADR-0019), `agent-discovery-contract`, `services-proxy-behavioral` (surfaces GAP-008 scope hierarchy not implemented + GAP-009 `validateScope` ↔ `grantScopes` self-contradiction; static SSRF resilience gate on `req.body` destructure), `services-execute-behavioral` (surfaces GAP-010 service catalog seed dead at boot), `ask-endpoint-behavioral` (cardinal cost-control: no LLM call when no services connected), `handshake-flow-behavioral` (surfaces GAP-011 duplicate `/handshakes/:id/status` handler at `src/index.js:7847`; pins GAP-005 `'owner'` literal), `connectors-master-only`, `agent-capabilities-end-to-end` (composite walk through token mint → discover → use → boundary). **F6.2** `scripts/agent-walkthrough.mjs` — bearer-only Node CLI walking the runbook's 8 phases programmatically, with credential-leak markers + SSRF probes + handshake e2e. New `npm run smoke:walkthrough`. **F6.3 / F6.4 / F6.5** four L2 live-smoke files gated on `SMOKE_URL` (skip silently in `npm test`): `agent-discovery-live-smoke`, `services-proxy-google-live-smoke`, `services-proxy-github-live-smoke`, `skills-and-handshakes-live-smoke`. New `npm run smoke:agent` / `smoke:google` / `smoke:github`. (`oauth-connect-flow-live-smoke` deferred — M3 wrap-up smoke already covered the connect flow live.) **F6.6** ADR-0020 ratified Option C (read-only spike); shipped `src/lib/schemas/connector-spec.js` (0-dep `GenericOAuthAdapter` validator enforcing F4 / ADR-0018 identity-vs-service scope split with no overlap) + handler wiring at `src/index.js:6389-6411` + 7-test spike covering round-trip / 4 schema rejections / scope gate fires before schema gate / non-spike types bypass. G0.3 snapshot regenerated for +19 line drift. **F6.7** triage table at the top of `capability-gaps.md`; 3 follow-up briefs filed (`F8-gateway-context-vs-llms-txt-mismatch`, `F9-scope-hierarchy-engine`, `F10-service-catalog-seed`); GAP-001/006 bundled into M14, GAP-005/011 into M6, GAP-004 into F7 (already), GAP-007 accept-risk via ADR-0016 follow-up. **Test baseline:** 55 / 56 / 767 → **64 / 69 / 855** (+9 suites, +88 passing tests, 5 L2 suites correctly skip when env vars unset, 28 / 28 snapshots stable, exit 0, full sweep ~64 s on Windows). Global progress total: 136 / 31 → 136 / 37. F6 milestone closes; F8 / F9 / F10 land in next sessions. |
| 2026-04-28 | planning | **F6 — Agent capability verification opened.** New milestone (cross-cutting follow-up, separate from M-numbered roadmap) tracking the test push that maps every agent-facing capability and verifies it end-to-end across L1 (in-process supertest, runs in `npm test`), L2 (gated live-smoke against `docker:smoke` with real Google + GitHub creds), and L3 (manual agent-as-user runbook + walkthrough script). F6.0 landed: `.context/capability-gaps.md` (schema + severity guide + 7 pre-seeded gap rows from the exploration pass — `ENABLE_OAUTH_*` docs/code drift, `/api/v1/google/*` no-auth mount, `/gateway/context` master-vs-llms.txt mismatch, missing marketplace `connector` type, handshake `user_id='owner'` literal, `/services/available` lacks scope, pre-M3 `provider_subject` NULL rows). F6.2 runbook and F6.6 ADR-0020 landed as markdown. F6 task brief filed at `.context/tasks/backlog/F6-agent-capability-verification.md`. F6.1 (L1 supertest), F6.3–F6.5 (L2 live-smoke), F6.6 spike test, F6.7 triage all wait on agent mode. Plan: `.cursor/plans/capability_test_plan_b4523725.plan.md`. Global progress total bumped 128 → 136; F6 row added at 1/8. |
| 2026-05-04 | implementation | **F12 / M-import bootstrap landed.** Phase B.1 of the upstream backport work shipped: ADR-0024 (`upstream-import-policy`) ratifies the per-import workflow + 8-section mini-plan template + ADR-0023..0029 reservations + stop conditions. New `.context/tasks/backlog/F12-upstream-import-2026-05.md` parent task with sub-row table (one row per matrix candidate). New `M-import` milestone in the Global progress table (empty until first mini-plan accepts). New `scripts/upstream-audit.mjs` (read-only, ~150 LOC, no new deps, supports `--json` + `--content-check`). Verify-pass run for the 10 "verify" rows from the matrix; per-row verdicts recorded in F12 task notes. New `.context/imports/` directory with README index + ~18 per-candidate mini-plans (8 strong + 0-5 verify-promoted + 2 architectural-deferred + 5 capability-proposed-or-deferred). NO cherry-picks landed in this commit; each mini-plan is at `Status: proposed` (or `deferred` for architectural / capability rows that need a follow-up ADR). Test gate sanity check: still 71/77 suites, 898 tests, 28/28 snapshots, exit 0 (no runtime code touched). Global progress total bumped 137 → 159 (F12 +22 sub-rows, M-import +0 until first accept). Direct-to-main commit per ADR-0024 §3 default delivery mode. |
| 2026-05-04 | release | **v0.6.0 cut as first public Docker release to GHCR.** ADR-0023 (`release-versioning-and-publishing`) ratifies milestone-aligned semver (M0..M5 done = v0.6.0; v1.0.0 reserved for SOC2/M7-finish), GHCR public registry (`ghcr.io/valerok/myapi-open`), root `Dockerfile` as the release image (multi-arch `linux/amd64` + `linux/arm64`), additive to existing `deploy.yml`. New `.github/workflows/release.yml` (tag-triggered, runs the test gate, multi-arch buildx, version-vs-tag verification, Trivy scan, GitHub Release with CHANGELOG body), `docker-compose.release.yml` + `.env.release.example` (one-line user spin-up; container exits 1 if any of the four required secrets is missing/banned), `CHANGELOG.md` v0.6.0 section. Bumped both `package.json` files 1.0.0/0.1.0 → 0.6.0. README "Option Zero: Pull the published release" section added. Tag `v0.6.0` pushed (commit `dc25fe5`); release workflow run 25304034880 triggered. NOTE: first GHCR push creates the package as private — one-time manual UI step at `https://github.com/users/ValeroK/packages/container/myapi-open/settings` to flip visibility to public so anonymous `docker pull` works (per ADR-0023 § "Operational changes required"). |

...
| M4 | Session + rate-limit dual-driver store | 9 | 2 | **In progress.** T4.1 + T4.2 done 2026-04-27 (`src/infra/session/` + 20-test driver-agnostic contract suite + upstream timer-leak neutralized in our wrapper). T4.4 next — swap inline wiring in `src/index.js` for `createSessionStore({ env, db })`. **Pre-stage gates required and green:** Stage-0 baseline (G0.1, G0.2, G0.3, G0.4, G0.6) + M4 layer (G4.1, G4.2). Every M4 PR that touches `src/index.js` MUST update the snapshots in G4.1 / G4.2 inline with the source change so reviewers can audit each deletion / replacement against the locked baseline. |
...
| T4.4 | `[ ]` Replace inline `sessionStore` wiring in `src/index.js` (lines 854-1177) with `createSessionStore({ env, db })` from `src/infra/session/` | S | T4.2 | T4.3 not required: factory already covers Redis-throws-loudly case. G4.2 snapshots will be intentionally updated by this task. |

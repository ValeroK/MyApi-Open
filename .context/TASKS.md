...
| T2.10 | `[ ]` Remove `crypto-js` from `src/package.json` `dependencies` (and drop `src/package-lock.json` if it exists) **or** delete the nested `src/package.json` + lockfile entirely if no other live script references them. The inventory test's "not resolvable from repo root" gate already enforces this from the root; this task closes the nested install surface. | S | T2.9 | Decide between trimming vs. deleting based on whether any remaining `src/` script (e.g. `qa`, `qa:security`) is still used in practice. |

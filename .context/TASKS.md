...
| M3 | OAuth state + PKCE + callback hardening | 10 | 1 | 🟡 In progress — Step 1 / T3.0 (inventory + ADR-0014) done 2026-04-21; Steps 2–8 / T3.1–T3.9 pending |
...
| T3.1 | `[ ]` Ensure `state_tokens` schema has columns: `state`, `user_id`, `mode`, `return_to`, `code_verifier`, `expires_at`, `used_at`, `created_at`. Add migration if missing | S | T0.7, T3.0 | **M3 Step 2 per ADR-0014.** Schema-gap assertions in `oauth-state-inventory.test.js` flip from `toBe(false)` → `toBe(true)` in this commit. |

...
| M4 | Session + rate-limit dual-driver store | 9 | 3 | **In progress.** T4.1 + T4.2 + T4.4 done 2026-04-27. Session-store path is now fully behind `createSessionStore({ env, db })`; `src/index.js` no longer requires `better-sqlite3-session-store` directly and the upstream timer-leak is neutralized at the boundary. T4.5 next — `RateLimitStore` interface + memory + sqlite drivers. **Pre-stage gates required and green:** Stage-0 baseline (G0.1, G0.2, G0.3, G0.4, G0.6) + M4 layer (G4.1, G4.2). Every M4 PR that touches `src/index.js` MUST update the snapshots in G4.1 / G4.2 inline with the source change so reviewers can audit each deletion / replacement against the locked baseline. |
